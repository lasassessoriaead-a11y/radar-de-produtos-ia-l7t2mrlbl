...
export default async function handler(req:any,res:any){
  ...
  try{
      ...
      try{
        ...
        for(const item of listings.slice(0,5)){
          ...
          rows.push({
            ...
            niche_key:String(item?.category_id||'mercadolivre_trends').toLowerCase(),
            search_query:keyword,
            external_id:externalId,
            title,
            image_url:String(item?.thumbnail||item?.secure_thumbnail||'').replace(/^http:/,'https:'),
            ...
            reason:{source:'mercadolivre_trends_listing_search_manual',trend_keyword:keyword,trend_position:i+1,free_shipping:freeShipping,conversion_learning_enabled:false,affiliate_link_required_after_selection:true,autonomous_prepare_enabled:false},
    ...
    return res.status(200).json({success:true,trend_keywords:keywords.length,listings_checked:listingsChecked,candidates:selected.length,saved:selected.length,skipped_upstream:skippedUpstream,source:'mercadolivre_trends_listing_search_manual'})
