...
async function mlJson(url:string,token:string){const r=await fetch(url,{headers:{Authorization:`Bearer ${token}`,Accept:'application/json','User-Agent':'RadarIA/1.0'}});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d?.message||d?.error||`Mercado Livre HTTP ${r.status}`);return d}
  ...
  try{
    ...
    for(let i=0;i<keywords.length;i++){
      ...
      for(const product of Array.isArray(found?.results)?found.results:[]){
        ...
        const offers=await mlJson(`https://api.mercadolibre.com/products/${encodeURIComponent(catalogId)}/items?limit=20`,session.access_token)
    ...
    return res.status(200).json({success:true,trend_keywords:keywords.length,candidates:rows.length,saved:rows.length,source:'mercadolivre_trends_api_manual'})
