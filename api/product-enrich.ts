...
export default async function handler(req: any, res: any) {
  ...
  const suppliedIds =
    body.shopee_ids && body.shopee_ids.shopid && body.shopee_ids.itemid
  ...
  const titleQuery = detectedTitle ? normWords(detectedTitle).slice(0, 10).join(' ') : ''
  const queries = [
    ...(ids
      ? [`site:shopee.com.br "${ids.itemid}"`, `site:shopee.com.br "${ids.shopid}" "${ids.itemid}"`]
      : []),
    ...(detectedTitle
      ? [`site:shopee.com.br "${detectedTitle}"`, `site:shopee.com.br ${titleQuery}`]
      : []),
    ...
    best =
      items
        ...
        .map((x: any) => {
          ...
          const sim = detectedTitle
            ? titleSimilarity(cleanTitle(String(x.title || '')), detectedTitle)
            : 0
          return { x, itemMatch, sim }
        })
        .filter((r: any) => r.itemMatch || r.sim >= 0.62 || (!ids && !detectedTitle))
        .sort((a: any, b: any) => Number(b.itemMatch) - Number(a.itemMatch) || b.sim - a.sim)[0]
        ?.x || null
    if (best) break
  }
  ...
  const priceRaw =
    meta['product:price:amount'] || meta['og:price:amount'] || offer.price || offer.lowprice || ''
  const metaPrice =
    Number(
      String(priceRaw)
        .replace(/[^0-9,.-]/g, '')
  ...
  const exactItem = Boolean(
    ids &&
    [best.link, best.title, best.snippet, JSON.stringify(best.pagemap || {})]
      .join(' ')
      .includes(ids.itemid),
  )
  ...
  if (!highConfidence || !title || !image || price <= 0) {
    return res.status(200).json({
      ...
      message:
        'Encontrei referências ao produto, mas ainda faltam dados confiáveis para importação automática.',
    })
  }
