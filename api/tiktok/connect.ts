...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!APP_KEY) return res.status(503).json({ success: false, error: 'TikTok Shop App Key ainda não configurada no servidor.' })

    const state = crypto.randomBytes(32).toString('hex')
    const value = encode({ state, user_id: session.user.id, created_at: Date.now() })
    res.setHeader('Set-Cookie', `${COOKIE}=${value}; Path=/api/tiktok; HttpOnly; Secure; SameSite=Lax; Max-Age=600`)
    const authorization_url = `https://shop.tiktok.com/alliance/creator/auth?app_key=${encodeURIComponent(APP_KEY)}&state=${encodeURIComponent(state)}`
