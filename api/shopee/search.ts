...
const HAIR_ACCESSORY_TERMS = [
  ...
  'milk', 'hair milk', 'tratamento', 'queratina', 'keratin'
]

const HAIR_DRYER_HARDWARE_SIGNALS = [
  '220v', '127v', '110v', 'bivolt', 'watt', ' watts', '2000w', '2100w', '2200w',
  'motor', 'ionico', 'ion ', 'ceramico', 'turmalina', 'profissional', 'potencia',
  'velocidade', 'temperatura', 'voltagem'
]

function isHairDryerSearch(keyword: string) {
  const q = normalize(keyword)
  return q.includes('secador') && q.includes('cabelo')
}

function isHairDryerCandidate(title: string) {
  const t = normalize(title)
  const startsLikeDryer =
    t.startsWith('secador de cabelo') ||
    t.startsWith('secador cabelo') ||
    t.startsWith('secador profissional') ||
    t.startsWith('secador portatil')
  const mentionsDryer = t.includes('secador de cabelo') || t.includes('secador cabelo')
  const hasAccessory = HAIR_ACCESSORY_TERMS.some(term => t.includes(term))
  const hardwareCount = HAIR_DRYER_HARDWARE_SIGNALS.filter(term => t.includes(term)).length
  const usageContext = /para secador de cabelo|uso com secador|compativel com secador/.test(t)

  if (usageContext) return false
  if (hasAccessory && hardwareCount === 0) return false
  if (startsLikeDryer) return true
  return mentionsDryer && hardwareCount >= 1
}
...
function intentFitScore(keyword: string, title: string) {
  ...
  if (isHairDryerSearch(keyword)) {
    const t = normalize(title)
    if (!isHairDryerCandidate(title)) return 0
    if (t.startsWith('secador de cabelo') || t.startsWith('secador cabelo')) score += 25
    const hardwareCount = HAIR_DRYER_HARDWARE_SIGNALS.filter(term => t.includes(term)).length
    score += Math.min(15, hardwareCount * 5)
  }
  return clamp(score, 0, 100)
}
...
export default async function handler(req:any,res:any) {
  ...
  try {
    ...
    products.sort((a:any,b:any)=>{ const rd=Number(b.raw_data?.relevance_score||0)-Number(a.raw_data?.relevance_score||0); if(Math.abs(rd)>=5)return rd; return b.opportunity_score-a.opportunity_score })
