...
export default async function handler(req:any,res:any) {
  ...
  try {
    ...
    let products=nodes.map((p:any)=>{
      const price=num(p.priceMin||p.priceMax), sales=num(p.sales), rating=num(p.ratingStar), commissionRate=ratePercent(p.commissionRate), commission=num(p.commission), relevance=relevanceScore(keyword,String(p.productName||'')), score=opportunityScore(sales,rating,commissionRate,price,relevance)
    ...
    // Keep partial matches available, but rank exact query intent first. This avoids accessories
    // mentioning the search phrase outranking the actual requested product.
    products.sort((a:any,b:any)=>{ const rd=Number(b.raw_data?.relevance_score||0)-Number(a.raw_data?.relevance_score||0); if(Math.abs(rd)>=10)return rd; return b.opportunity_score-a.opportunity_score })
    const pageInfo=result?.pageInfo||{}
    return res.status(200).json({success:true,marketplace:'Shopee',status:'ok',message:`${products.length} produtos encontrados pela Shopee Affiliate Open API.`,total_found:products.length,products,page,offset:(page-1)*limit,next_offset:page*limit,has_more:Boolean(pageInfo?.hasNextPage),page_info:pageInfo,source_mode:'shopee_affiliate_open_api'})
