...
async function mlJson(url: string, accessToken: string) {
  ...
  const data = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(data?.message || data?.error || `Mercado Livre HTTP ${res.status}`)
  return data
}

async function refreshSession(session: MlSession, clientSecret: string, appId: string) {
  if (session.expires_at && Date.now() < Number(session.expires_at) - 120_000) {
  ...
  if (!res.ok || !data?.access_token) {
    throw new Error(data?.message || data?.error || 'Falha ao renovar Mercado Livre.')
  }
...
function scoreCandidate(trendPosition: number, sold: number, price: number, competitors: number, freeShipping: boolean) {
  ...
  if (sold >= 1000) score += 15
  else if (sold >= 500) score += 12
  else if (sold >= 100) score += 9
  else if (sold >= 20) score += 5
  if (price >= 25 && price <= 600) score += 5
  if (freeShipping) score += 4
  if (competitors >= 80) score -= 5
  else if (competitors > 0 && competitors <= 15) score += 4
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!claim?.allowed) {
      return res.status(200).json({ success: true, skipped: true, reason: 'cooldown' })
    }
    claimed = true
    ...
    for (const connection of connections) {
      ...
      if (refreshed.refreshed) {
        await bridge(oidc, {
          ...
          credentials_encrypted: encryptMlSession(session, clientSecret),
        })
      }

      const trends = await mlJson('https://api.mercadolibre.com/trends/MLB', session.access_token)
      const keywords = (Array.isArray(trends) ? trends : [])
      ...
      for (let trendIndex = 0; trendIndex < keywords.length; trendIndex++) {
        ...
        const found = await mlJson(searchUrl.toString(), session.access_token)
        const catalog = Array.isArray(found?.results) ? found.results : []

        for (const product of catalog) {
          const catalogId = String(product?.id || '')
          if (!catalogId) continue
          const offers = await mlJson(
            `https://api.mercadolibre.com/products/${encodeURIComponent(catalogId)}/items?limit=20`,
          ...
          for (const offer of candidates) {
            const item = await mlJson(
              `https://api.mercadolibre.com/items/${encodeURIComponent(String(offer.item_id))}`,
              session.access_token,
            )
          ...
          const sold = Number(best.sold_quantity || 0)
          const competitors = Number(offers?.paging?.total || offerRows.length || 0)
          const proactiveScore = scoreCandidate(
            trendIndex + 1,
            sold,
            price,
            competitors,
            Boolean(best?.shipping?.free_shipping),
          )
          ...
          const row = {
            user_id: connection.user_id,
            niche_key: String(product?.domain_id || best?.category_id || 'mercadolivre_trends').toLowerCase(),
            search_query: keyword,
            external_id: externalId,
            title: String(best.title || product?.name || keyword),
            image_url: String(best?.pictures?.[0]?.secure_url || best?.thumbnail || '').replace(/^http:/, 'https:'),
            product_url: String(best.permalink),
            affiliate_url: null,
            price,
            commission_rate: 0,
            commission_amount: 0,
            sales_count: sold,
            rating: 0,
            base_score: proactiveScore,
            niche_confidence: 0,
            proactive_score: proactiveScore,
            reason: {
              source: 'mercadolivre_trends_api',
              trend_keyword: keyword,
              trend_position: trendIndex + 1,
              catalog_product_id: catalogId,
              competitors,
              free_shipping: Boolean(best?.shipping?.free_shipping),
              conversion_learning_enabled: false,
              affiliate_link_required_after_selection: true,
              autonomous_prepare_enabled: false,
            },
          }
    ...
    const candidates = [...allCandidates.values()]
      .sort((a, b) => Number(b.proactive_score) - Number(a.proactive_score))
      .slice(0, Math.max(10, connections.length * 20))
    ...
    if (claimed) {
      try {
        await bridge(oidc, { mode: 'fail', run_id: runId, error: message, stage: 'mercadolivre_trends' })
      } catch {}
    }
